load_files = function(genofile, phenofile, testsfile, gwasfile = NULL, infofile = NULL){
 #(1) Load genotype and phenotype files
 ...
 #(2) Load Test sample list file
  test <- read.table(testsfile, header=T, check.names=F)
  test <- as.character(test[,1])
  test <- intersect(rownames(pheno1), test)
  train <- setdiff(rownames(pheno1), test) 
  cat(paste0("Test sample: Number of test samples is ", length(test)), '\n')
  cat(paste0("Train sample: Number of train samples is ", length(train)), '\n\n')
  ...
  if (is.null(gwasfile)){
    ...
    write.csv(gwas_results, file = "GWAS_marker_pvalue.csv", quote=F, row.names=F)
  ...
  if (dim(geno1)[2] == 0){  cat(paste0("Error6: Marker names is not matched between  Genotype file and GWAS file."))
    quit(save="no")
  }
  if (is.null(gwasfile)){
    return(list(genotype = geno1, phenotype = phenotype, trainsample = train, testsample = test, pheno = pheno1, gwas = gwas_results, info = infotable, ix = ix))
  } else {
    return(list(genotype = geno1, phenotype = phenotype, trainsample = train, testsample = test, pheno = data.frame(pheno1, check.names=F), gwas = gwas_results, ix = ix))
  }
}
