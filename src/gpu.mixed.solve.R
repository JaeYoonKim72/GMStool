gpu.mixed.solve <- function (y, Z = NULL, K = NULL, X = NULL, method = "REML", 
          bounds = c(1e-09, 1e+09), SE = FALSE, return.Hinv = FALSE) 
{
  ...
  if (is.null(X)) {
    p <- 1
    X <- gpuMatrix(rep(1, n), n, 1)
  }
  p <- ncol(X)
  if (is.null(p)) {
    p <- 1
    X <- gpuMatrix(X, length(X), 1)
  }
  if (is.null(Z)) {
    Z <- diag(n)
  }
