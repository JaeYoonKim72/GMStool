...
parser$add_argument("-c", "--corr", type = "double", default = 0.95, help = "Target correlation rate for the validation set. If the correlation rate for the validation set reaches the target correlation rate, the marker selection is stopped and the results up to that point is returned (Defualt 0.95)")
parser$add_argument("-d", "--increment", type = "double", default = 0.00005, help = "Increament of correlation rate. In order for a marker to be selected, the correlation rate of the marker must be greater than or equal to the value of the correlation rate of the immediately previously selected marker plus this increment value. This condition is applied to both the train and validation set, so the marker is selected only when satisfying this condition for both sets at the same time (Default 0.00005)")
parser$add_argument("-x", "--stopx", type = "double", default = 0.2, help = "Stop condition: x is a number corresponding to X percent of the total number of input markers. If the correlation rates of train and validation sets do not improved x times in a row, the marker selection for the corresponding CV is stopped and the results up to that point are returned (Default 0.2)")
...
cat("13) # of SNPs to be selected at one time is", sel_snps, "\n")
